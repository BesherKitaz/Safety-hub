...
export default function VerifyEmail() {
  ...
  const emailChange = params.get("purpose") === "email-change";
  ...
  useEffect(() => {
    ...
    const confirm = async () => {
      try {
        if (emailChange) {
          await api.post("/api/user/email-change/confirm", { token });
          if (active) navigate("/user", { replace: true });
          return;
        }
    ...
    confirm();
    return () => { active = false; };
  }, [emailChange, navigate, reset, token]);
